# this code is for graphdb works with good rendered 3D model, latest code April 15 2025
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _setup_base_plot(self):
        """ Sets up the static parts of the plot: mesh, reference spheres, axes, lighting, shading. """
        print("Setting up base plot...") # Add print statement for debugging
        self.plotter.clear() # Clear previous actors if any
        ...
        try:
            ...
            # IMPORTANT: Compute normals for proper shading/lighting
            self.airfoil_mesh.compute_normals(cell_normals=False, point_normals=True,
                                              split_vertices=True, flip_normals=False,
                                              consistent_normals=True, auto_orient_normals=True,
                                              inplace=True)
            print(f"STL loaded. Has point normals: {self.airfoil_mesh.point_data.active_normals is not None}")

            # --- Add Main Mesh with Smooth Shading ---
            # Ensure opacity is 1.0 and enable smooth shading
            self.plotter.add_mesh(
                self.airfoil_mesh,
                color="lightgray",
                opacity=1.0,
                smooth_shading=True, # Explicitly enable smooth shading
                specular=0.3,        # Add a bit of shininess
                specular_power=15,
                name="airfoil_base_mesh" # Give it a name for potential later reference
            )
            print("Airfoil mesh added with smooth shading.")

        except FileNotFoundError:
             QMessageBox.critical(self, "File Error", f"Cannot find STL file: {stl_file}")
             self.airfoil_mesh = pv.Sphere() # Placeholder
             self.plotter.add_mesh(self.airfoil_mesh, color="red", smooth_shading=True)
        except Exception as e:
            print(f"Error loading or processing mesh: {e}")
            QMessageBox.critical(self, "Mesh Error", f"Error loading/processing STL: {e}")
            self.airfoil_mesh = pv.Sphere()
            self.plotter.add_mesh(self.airfoil_mesh, color="red", smooth_shading=True)


        # --- Add Lights (Optional but recommended) ---
        # You might want to remove default lights if adding your own complex setup
        #self.plotter.remove_lights()
        # Example: Add a couple of lights from different angles
        light_top = pv.Light(position=(0, 50, 150), light_type='scene light', intensity=0.8)
        light_side = pv.Light(position=(100, 50, 50), light_type='scene light', intensity=0.6)
        self.plotter.add_light(light_top)
        self.plotter.add_light(light_side)
        # Or just use the default lightkit which is usually reasonable:
        self.plotter.enable_lightkit()
        print("Lighting enabled.")


        # --- Reference Spheres ---
        # Make sure they are also opaque if you want them to cast/receive shadows properly
        for label, center in reference_spheres.items():
            sphere = pv.Sphere(radius=0.3, center=center)
            self.plotter.add_mesh(sphere, color="yellow", opacity=1.0, name=f"ref_{label}", smooth_shading=True) # Added smooth shading
            # Adjusted label params for better visibility against potentially complex backgrounds
            self.plotter.add_point_labels(
                [center + np.array([0,0,0.5])],
                [label.replace("_", " ").title()],
                font_size=10,
                text_color="black", # Black text often more readable
                point_color="yellow",
                shape_color='gray', # Give labels a slight background
                shape_opacity=0.5,
                always_visible=False, # Maybe don't force visibility initially
                name=f"ref_label_{label}"
            )
        print("Reference spheres added.")
        ...
        # --- Set Initial View ---
        self.set_iso_view() # This should call reset_camera and render

        print("Base plot setup complete.")
        # Ensure a render happens after setup if set_iso_view doesn't guarantee it
        self.plotter.render()
